...
int main(int argc, const char *argv[])
{
    ...
    const char *file_name = "./NYCU_logo.bmp";
    ...
    cvtColor(image, image2, cv::COLOR_BGR2BGR565);
    ...
    for (int y = 0; y < image_size.height; y++)
    {
        ...
        ofs.write(reinterpret_cast<char*>(image2.ptr(y)), image_size.width * (fb_info.bits_per_pixel / 8));
    }
...
struct framebuffer_info get_framebuffer_info(const char *framebuffer_device_path)
{
    struct framebuffer_info fb_info = {0};        // Used to return the required attrs.
